(function () {
  ...
  var API_ENDPOINT = '/api/reactions';
  var CACHE_EXPIRY = 5 * 60 * 1000; // 5 minutes

  function getCacheKey() {
    return PAGE_KEY + ':cache';
  }

  function getCacheTimestamp() {
    return PAGE_KEY + ':timestamp';
  }
  ...
  function cacheData(data) {
    try { 
      ...
      localStorage.setItem(getCacheTimestamp(), Date.now());
    } catch (e) {}
  }
  ...
  function shouldRefetch() {
    try {
      var timestamp = localStorage.getItem(getCacheTimestamp());
  ...
  function fetchDataFromServer() {
    return fetch(API_ENDPOINT + '?pathname=' + encodeURIComponent(location.pathname))
      .then(function (res) {
  ...
  function loadDataAsync() {
    ...
    return Promise.resolve(loadCachedData());
  }
  ...
  function saveData(data) {
    ...
    // 异步保存到服务器
    fetch(API_ENDPOINT + '?pathname=' + encodeURIComponent(location.pathname), {
      method: 'POST',
  ...
  function renderReactions() {
    ...
    // 异步从服务器加载最新数据
    loadDataAsync().then(function (serverData) {
      if (JSON.stringify(serverData) !== JSON.stringify(data)) {
        // 如果服务器数据不同，更新本地显示
        cacheData(serverData);
        REACTIONS.forEach(function (r) {
          var d = serverData[r.key] || { count: 0, reacted: false };
          var btn = buttons[r.key];
          if (btn) {
            btn.className = 'reaction-btn' + (d.reacted ? ' reacted' : '');
            btn.querySelector('.count').textContent = d.count || '';
            if (r.key === 'igiari') {
              igiariNotice.style.display = d.reacted ? 'block' : 'none';
            }
          }
        });
      }
